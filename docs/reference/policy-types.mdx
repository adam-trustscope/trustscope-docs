...
sidebar_position: 2
title: Policy Types
description: Complete reference of 24 TrustScope policy types for AI governance
---

# Policy Types

TrustScope provides 24 policy types — configurable rules you define to govern agent behavior.
...
| **Protect** | +9 (12 total) | blocked_values, rate_limit, time_window, require_field, recipient_allowlist, amount_limit, require_approval, block_action, cumulative_limit |
| **Enforce** | +9 (21 total) | human_approval, chain_depth_limit, external_check, per_entity_limit, custom_logic, semantic_rule, delegation_restriction, delegation_limits, exposure_threshold |
| **Govern** | +3 (24 total) | custom_function, multi_approval, tool_provenance |

### Enforcement Modes

Every policy supports these enforcement modes:
...
## Protect Tier (+9 Types)

### Blocked Values (`blocked_values`)

Blocklist specific parameter values.
...
1. **Create policy** in `off` mode — no impact on traffic
2. **Enable simulate** — policy evaluates, logs results, no enforcement
3. **Review simulate results** — confirm no false positives
4. **Switch to alert** — policy flags violations, sends notifications
5. **Activate block** — policy enforces on live traffic
