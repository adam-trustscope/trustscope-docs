...
**Version:** 0.3.0 · **Package:** `trustscope` · **Python:** 3.8+
...
ts = TrustScope(
    api_key="ts_live_xxx",        # Required (or set TRUSTSCOPE_API_KEY env var)
    base_url="https://api.trustscope.ai",  # Optional override
    timeout=30.0,                  # Request timeout in seconds
    fail_open=True,                # Default: if TrustScope is unavailable, allow actions
)
```

:::info Fail-Open Default
`fail_open=True` means your agent continues working even if TrustScope is unavailable. Set `fail_open=False` for strict enforcement where actions are blocked when TrustScope can't be reached.
:::
...
session.log_llm_call(provider="openai", model="gpt-5.2", ...)
...
Reference policies by their **name** (created in dashboard or via API):

```python
# Reference your named policies
policies=["no-pii", "no-profanity", "content-filter"]
policies=["cost-limit-10", "token-limit-4000"]
policies=["business-hours", "max-refund-1000"]
```

:::info Policy Names vs Types
In `@ts.enforce()`, you reference policies by **name** (e.g., `"cost-limit-10"`).
When creating policies, you specify a **type** (e.g., `max_value`).

See [Policy Types](/docs/reference/policy-types) for available types.
:::
...
response = client.messages.create(
    model="claude-sonnet-4-5-20251124",
    max_tokens=1024,
    messages=[{"role": "user", "content": "Hello"}]
)
```
